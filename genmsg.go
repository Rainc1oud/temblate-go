...
import tmblg "github.com/Rainc1oud/temblate-go"
...
func GetMessage(lang, key string, data interface{}) string{
	return tmblg.GetMessage(lang, key , data)
}
`

func Generate(tgtfile, tmpldir string) error {
	msgs := fmt.Sprintf(goTmpl, *readTemplates(tmpldir))
	msgs = strings.ReplaceAll(msgs, "temblate_go.", "tmblg.") // somehow %#v adds the package scope, which we don't want
...
func readTemplates(dir string) *MessageTemplates {
	...
	fmt.Fprintf(os.Stderr, "INFO: got templates from %s:\n%+v\n", filepath.Join(pwd, dir), tfn)
	if err != nil {
